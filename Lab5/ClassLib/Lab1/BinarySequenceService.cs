...
public static class BinarySequenceService
{
    ...
    private static long FindR(long n)
    {
        long left = 2;
        long right = 1000000;
        long r = 0;
    ...
    private static long BinomialCoefficient(long n, int k)
    {
        if (k < 0 || k > n)
            return 0; 
        if (k == 0 || k == n)
    ...
    private static (long p, long q) FindPQ(long r, long nPrime)
    {
        for(long q = 1; q < r; q++)
        {
            for(long p = 0; p < q; p++)
    ...
    private static BigInteger ComputeNumber(long p, long q, long r)
    {
